(() => {
  ...
  const cancelRegistrationButton = document.querySelector("#cancelRegistrationButton");
  const houseCheckMessage = document.querySelector("#houseCheckMessage");
  ...
  async function callApi(payload) {
    if (!config.apiBaseUrl) {
      throw new Error("ยังไม่ได้ตั้งค่า API URL");
    }
  ...
  function showRegistration(member) {
    ...
    const lineName = document.querySelector("#registerLineName");
    const linePicture = document.querySelector("#registerLinePicture");
  ...
  async function handleLineCallback() {
    ...
    const lineErrorDescription = params.get("error_description");

    if (lineError) {
      cleanCallbackUrl();
      showToast(lineErrorDescription || "ผู้ใช้ยกเลิกการเข้าสู่ระบบด้วย LINE");
      return;
    }

    if (!code) return;

    setButtonLoading(lineLoginButton, true, "กำลังตรวจสอบข้อมูล...");

    try {
      const savedState = sessionStorage.getItem(OAUTH_STATE_KEY);

      if (!savedState || !returnedState || returnedState !== savedState) {
        throw new Error("ตรวจสอบความถูกต้องของ LINE Login ไม่สำเร็จ กรุณาลองใหม่");
      }
      ...
      if (!result.sessionToken || !result.member) {
        throw new Error("API ไม่ได้ส่งข้อมูล Login กลับมาครบถ้วน");
      }
  ...
  async function checkHouseNumber() {
    ...
    setButtonLoading(checkHouseButton, true, "กำลังตรวจ...");
  ...
  function validateRegistrationForm() {
    ...
    const representativeStatus = String(
      document.querySelector("#representativeStatus")?.value || ""
    ).trim();
    const lineId = String(document.querySelector("#lineId")?.value || "").trim();
    const email = String(document.querySelector("#email")?.value || "").trim();
    const pdpaConsent = document.querySelector("#pdpaConsent")?.checked === true;
    const certification = document.querySelector("#certification")?.checked === true;

    if (!/^\d{1,3}$/.test(houseNumber) || Number(houseNumber) < 1 || Number(houseNumber) > 364) {
      throw new Error("กรุณากรอกบ้านเลขที่ตั้งแต่ 1 ถึง 364");
    }
    if (!state.houseAvailable) throw new Error("กรุณากดตรวจสอบบ้านเลขที่ก่อนลงทะเบียน");
    if (!fullName) throw new Error("กรุณากรอกชื่อ-นามสกุล");
    if (!/^0\d{8,9}$/.test(phone)) {
      throw new Error("กรุณากรอกเบอร์โทรศัพท์ให้ถูกต้อง เช่น 0812345678");
    }
    ...
    return {
      ...
      publicIp: "ตรวจสอบผ่าน Cloudflare",
  ...
  async function submitRegistration(event) {
    ...
    try {
      ...
      if (!result.registration) {
        throw new Error("ระบบบันทึกแล้วแต่ไม่พบข้อมูลสมาชิก");
      }

      showDashboard({
        registered: true,
        displayName: result.registration.fullName,
        lineDisplayName: state.member?.lineDisplayName || "",
        linePictureUrl: state.member?.linePictureUrl || "",
        houseNo: result.registration.houseNumber,
        status: result.registration.status || "Active",
        role: result.registration.role || "Member",
  ...
  async function restoreSession() {
    ...
    try {
      ...
      if (!result.authenticated || !result.member) {
        throw new Error(result.message || "Session หมดอายุ");
      }
  ...
  async function logoutFromSystem() {
    const sessionToken = getSessionToken();
    setButtonLoading(logoutButton, true, "กำลังออกจากระบบ...");
  ...
  function bindEvents() {
    ...
    demoLoginButton?.addEventListener("click", loginDemo);
    logoutButton?.addEventListener("click", logoutFromSystem);
    cancelRegistrationButton?.addEventListener("click", logoutFromSystem);
    checkHouseButton?.addEventListener("click", checkHouseNumber);
    ...
    document.querySelectorAll(".menu-card").forEach((button) => {
      button.addEventListener("click", () => {
        showToast(`โมดูล ${button.dataset.module || ""} จะพัฒนาในขั้นตอนถัดไป`);
      });
    });
  }
